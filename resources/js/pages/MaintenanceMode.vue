<script setup>
    ...
    function activate() {
        if (
            !confirm(__('Are you sure you want to activate maintenance mode? Visitors will see the maintenance page.'))
        ) {
            return;
        }
        ...
        fetch(props.storeUrl, {
            ...
            .catch((error) => {
                if (error.data?.errors) {
                    ...
                    Statamic.$toast.error(Array.isArray(firstError) ? firstError[0] : firstError);
                } else {
                    Statamic.$toast.error(__('Something went wrong'));
                    console.error(error);
                }
            ...
            <Card>
                <div class="flex items-center gap-4">
                    ...
                    <Description>
                        ...
                        <span v-if="isActive" class="font-semibold text-red-600">{{ __('active') }}</span
                        ><span v-else class="font-semibold text-green-600">{{ __('inactive') }}</span
                        >.
                    </Description>
                </div>
            </Card>
        ...
        <Panel v-if="isActive && secretUrl">
            ...
            <Card class="bg-amber-50 dark:bg-amber-950/30">
                ...
                <div class="flex items-center gap-2">
                    <Input type="text" readonly :model-value="secretUrl" class="flex-1 font-mono text-sm" />
